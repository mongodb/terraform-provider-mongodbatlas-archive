...
func resourceMongoDBAtlasProjectIPWhitelistImportState(d *schema.ResourceData, meta interface{}) ([]*schema.ResourceData, error) {
	...
	whitelistResp, _, err := conn.ProjectIPWhitelist.List(context.Background(), d.Id(), options)
	if err != nil {
		return nil, fmt.Errorf("Couldn't import ip whitelist %s in project_id %s, error: %s", whitelistResp, d.Id(), err.Error())
	}

	var entries []string
	whiteListMap(whitelistResp, func(entry string) {
	...
	if err := d.Set("whitelist", flattenProjectIPWhitelist(whitelistResp)); err != nil {
